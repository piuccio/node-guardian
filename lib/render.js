...
jspm.setPackagePath(path.join(__dirname, '../'));
...
function compileMainTemplate () {
	var mainTemplateText = read('../views/scraped.html');
	...
	jspm.bundleSFX('static/javascripts/front-main.js', '../build/javascripts/front-bundle.js', {
		mangle: false,
		sourceMaps: false
	}).then(function (res) {
		console.log(res)
...
exports.front = function (pressed) {
	...
	return fromDisk.mainTemplate.render({
		...
		frontClasses: '',
		frontScriptDev: 'javascripts/front-main.js',
		// frontScriptBundle: 'front-bundle',
	}, {
		head: '',
		body: fromDisk.frontBodyTemplate,
		signposting: '',
		breadcrumb: '',
		initialData: JSON.stringify(pressed)
	});
};
